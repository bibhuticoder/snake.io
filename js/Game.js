class Game{
	...
	drawMap(){
		...
		for (var i = 0; i < this.snakes.length; i++) {
			...
			this.ctxFood.arc(start.x + playerInMap.x, playerInMap.y + 10, 2, 0, 2*Math.PI);
			this.ctxFood.fill();
		}	
	}
	...
	firebaseConfig(){
		...
		var id = rootRef.push();
		this.snakes[0].id = id.key;
		console.log(id.key);

		// id.set({
		// 	name: this.snakes[0].name,
		// 	score: this.snakes[0].score
		// });
